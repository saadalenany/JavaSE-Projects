...
public class FinalFormDuality {
    ...
    public UndecoratorScene DrawFinalDual(Stage stage) {
        ...
        for(int i=0 ; i<ObjectiveF.size() ; i++){
            if(i==ObjectiveF.size()-1){
                hb1.getChildren().addAll(ObjectiveF.get(i),new Label("Y"+i));
            }else{
                hb1.getChildren().addAll(ObjectiveF.get(i),new Label("Y"+i) , new Label("+"));
            }
        }
        HBox contain = new HBox();
        contain.getChildren().addAll(MaxMin , new Label("W") , hb1);
        ...
        for(int i=0 ; i<constran.size() ; i++){
            for(int j=0 ; j<conperline.size() ; j++){
                if(i==constran.size()-1){
                    HBox hh = new HBox();
                    hh.getChildren().addAll(new Label(constran.get(i).get(j)+"") , new Label("Y"+j));
                    hbcon.add(hh);
                }else{
                    HBox hh = new HBox();
                    hh.getChildren().addAll(new Label(constran.get(i).get(j)+"") , new Label("Y"+j) , new Label("+"));
                    hbcon.add(hh);
                }
            }
        }
        ...
        for(int i=0 ; i<operators.size() ; i++){
            if(i==ObjectiveF.size()-1){
                HBox hh = new HBox();
                hh.getChildren().addAll(operators.get(i),new Label("Y"+i));
                hbcombo.add(hh);
            }else{
                HBox hh = new HBox();
                hh.getChildren().addAll(operators.get(i),new Label("Y"+i) , new Label("+"));
                hbcombo.add(hh);
            }
        }
        ...
        for(int i=0 ; i<RHS.size() ; i++){
            if(i==RHS.size()-1){
                HBox hh = new HBox();
                hh.getChildren().addAll(RHS.get(i),new Label("Y"+i));
                hbRHS.add(hh);
            }else{
                HBox hh = new HBox();
                hh.getChildren().addAll(RHS.get(i),new Label("Y"+i) , new Label("+"));
                hbRHS.add(hh);
            }
        }
